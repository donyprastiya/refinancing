...
export class DataService {
  ...
  addBooking(konsumen_nokontrak:string, konsumen_nama:string, konsumen_noHP:string, konsumen_alamat:string, otr:number, tenor:number, jenisAsuransi:string, dpPersen:number, dp:number, phAwal:number, rateAsuransi:number, asuransi:number, bungaFlat:number, bungaJual: number, tph:number, angsuran:number, provisiAdmin:number, fidusia:number, cp:number, potongan:number, totalPotongan:number, danaCair:number, danaCairMinET:number){
    let URL_API  = DOMAIN_API+"csf/savebooking";
    return this.http.post(URL_API, {konsumen_nokontrak, konsumen_nama, konsumen_noHP, konsumen_alamat, otr, tenor, jenisAsuransi, dpPersen, dp, phAwal, rateAsuransi, asuransi, bungaFlat, bungaJual, tph, angsuran, provisiAdmin, fidusia, cp, potongan, totalPotongan, danaCair, danaCairMinET}, header)
  }
